...
public class GameController : Singleton<GameController>
{
    ...
    private void FixedUpdate()
    {
        if(abridged)
        {
            ...
            if(abridgedTimeLeft <= 0)
            {
                ...
                Debug.Log(winner + " has won with a net worth of " + netWorth);
    ...
    public void OnArriveAtProperty(Property property)
    {
        ...
        {
            if (BankController.Instance.PropertyAvailable((PurchaseableProperty)property))
            {
                if (GetCurrentPlayer().IsAI())
                {
            ...
            else
            {
                ...
                if(!((PurchaseableProperty)property).IsMortgaged() && !owner.IsInJail())
                {
