...
public class BankController : Singleton<BankController>
{
    ...
    {
        SubtractBalance(balanceDuePlayer, balanceDue, true);
    }
    ...
    {
        foreach (PurchaseableProperty prop in unsoldProperties)
        {
            if(prop.GetGroup() == property.GetGroup() && Mathf.Abs(property.GetHouses() + 1 - prop.GetHouses()) > 1)
            {
                return false;
            }
        }
